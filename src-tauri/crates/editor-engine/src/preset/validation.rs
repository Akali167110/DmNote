use super::*;
use crate::models::*;
use crate::state::migration::migrate_legacy_sprite_wire;
pub fn decode_preset(content: &str) -> Result<PresetFile, String> {
    ...
    if let Some(detail) = invalid_position_style_detail(&value) {
        return Err(format!("invalid-preset: {detail}"));
    }
    default_preset_note_gradient_multipliers(&mut value);
    serde_json::from_value(value).map_err(|_| "invalid-preset".to_string())
}
...
pub fn invalid_position_style_detail(preset: &serde_json::Value) -> Option<String> {
