...
impl KeyCounterSettings {
    pub fn migrate_legacy_font_weight(&mut self) -> bool {
    ...
    pub fn canonicalize_gradient_pairs(&mut self) -> (bool, bool) {
...
// 제거된 텍스트 외곽선 필드 정리
pub fn scrub_removed_text_outline_fields(value: &mut serde_json::Value) -> bool {
    let mut changed = false;
...
impl KeyPosition {
    ...
    pub fn mirror_note_body_to_glow(&mut self) -> bool {
    ...
    pub fn canonicalize_image_mode(&mut self) -> bool {
...
pub struct NoteGradientShadow {
    pub color: NoteColor,
    pub opacity_top: u32,
    pub opacity_bottom: u32,
}

pub fn note_gradient_shadow(gradient: &GradientSpec, opacity: u32) -> Option<NoteGradientShadow> {
...
pub fn default_missing_note_gradient_multipliers(position: &mut serde_json::Value) -> bool {
    let Some(position) = position.as_object_mut() else {
        return false;
    };
    let mut changed = false;
...
pub fn note_border_representative_hex(color: &str) -> Option<String> {
...
pub fn compact_canonical_rgba(color: &str) -> String {
    let trimmed = color.trim();
    if let Some(hex) = trimmed.strip_prefix('#') {
