...
use crate::{
    css_history::{migrate_custom_css_history_timestamps, normalize_custom_css_history},
...
pub mod recovery;
use crate::portable_assets::{parse_portable_asset_reference, AssetCategory};
...
pub use normalization::{
    default_store_note_gradient_multipliers, has_convertible_note_border_color,
    has_explicit_invalid_element_id, has_legacy_font_weight_state, has_valid_selected_key_type,
    key_position_lengths_mismatch, migrate_legacy_knob_sensitivity,
    migrate_legacy_sprite_positions_value, migrate_sound_library_enabled,
    normalize_blank_font_colors, remove_legacy_panel_detach_setting, repair_editor_revision,
    repair_native_position_ranges, repair_semantic_identities, repair_sprite_numeric_ranges,
};
#[cfg(test)]
pub use recovery::{
...
pub use recovery::{repair_custom_tab_key_layout_pairs, repair_legacy_state};
...
pub struct LoadedStore {
    pub data: AppStoreData,
    pub needs_persist: bool,
    pub repaired: bool,
}

/// store 파일 로드 및 마이그레이션 적용
pub fn load_store_bytes(
    content: &[u8],
    source_label: &str,
    now: i64,
    canonicalize_legacy_css_path: impl Fn(&str) -> String,
) -> LoadedStore {
    ...
    needs_persist |= crate::css_history::migrate_custom_css_history_at_load(
    ...
    if needs_persist {
        log::info!("[Store] Persisting migrated store file at {}", source_label);
    }
    LoadedStore {
        data: state,
        needs_persist,
...
pub mod test_support {
    pub use super::normalization::rgba_to_hex;
    pub use super::recovery::{
        recover_collection_field, recover_key_mapping_entries, recover_local_font_enabled,
        recover_sound_library_entries,
    };
    pub const LEGACY_OVERLAY_WIDTH: f64 = super::LEGACY_OVERLAY_WIDTH;
    pub const LEGACY_OVERLAY_HEIGHT: f64 = super::LEGACY_OVERLAY_HEIGHT;
    pub const LEGACY_PANEL_DETACH_ENABLED_KEY: &str = super::LEGACY_PANEL_DETACH_ENABLED_KEY;
}
