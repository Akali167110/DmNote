...
pub fn paths_have_same_identity(left: &Path, right: &Path) -> bool {
    #[cfg(target_os = "windows")]
    {
...
pub fn file_url_to_path(value: &str) -> FileUrlPath {
...
fn file_path_from_url(url: &Url) -> Result<PathBuf, ()> {
    url.to_file_path()
}

// WASM에는 OS 경로 변환 API가 없다. 레거시 참조의 식별만 수행하며 파일을 열지 않는다.
