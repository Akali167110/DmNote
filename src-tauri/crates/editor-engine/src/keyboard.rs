...
use crate::models::{normalize_key_mappings, KeyMappings, KeySlot, SlotMatch};
...
impl KeyboardMatcher {
    ...
    pub fn set_mode(&mut self, mode: impl Into<String>) -> bool {
        let mode = mode.into();
    ...
    pub fn update_mappings(&mut self, mut mappings: KeyMappings) {
        normalize_key_mappings(&mut mappings);
        let state = &mut self.state;
        state.mappings = mappings;
        state.rebuild_compiled_state();
    ...
    #[cfg(test)]
    pub fn register_key_down(&mut self, mode: &str, key: &str) -> bool {
    ...
    #[cfg(test)]
    pub fn register_key_up(&mut self, mode: &str, key: &str) -> bool {
        if self.current_mode() != mode {
            return false;
        }
        let physical_id = format!("test:key:{key}");
...
pub struct WebKeyboardMatcher {
    matcher: KeyboardMatcher,
}
#[cfg_attr(target_arch = "wasm32", wasm_bindgen)]
impl WebKeyboardMatcher {
    #[cfg_attr(target_arch = "wasm32", wasm_bindgen(constructor))]
    pub fn new(mappings_json: &str, mode: &str) -> Result<Self, String> {
        Ok(Self {
            matcher: KeyboardMatcher::new(
                serde_json::from_str(mappings_json).map_err(|e| e.to_string())?,
                mode,
            ),
        })
    }
    pub fn update(&mut self, mappings_json: &str, mode: &str) -> Result<bool, String> {
        Ok(self.matcher.update_mappings_and_set_mode(
            serde_json::from_str(mappings_json).map_err(|e| e.to_string())?,
            mode,
        ))
    }
    pub fn feed(&mut self, input_json: &str) -> Result<String, String> {
        #[derive(Deserialize)]
        #[serde(rename_all = "camelCase")]
        struct Input {
        ...
        serde_json::to_string(&self.matcher.match_and_register(
            input.physical_id.as_deref(),
            input.device,
            input.candidates.iter().map(String::as_str),
            input.is_down,
        ))
        .map_err(|e| e.to_string())
    }
    pub fn active(&self) -> Result<String, String> {
        let (mode, keys) = self.matcher.current_mode_and_pressed_keys();
        serde_json::to_string(&serde_json::json!({"mode":mode,"keys":keys}))
            .map_err(|e| e.to_string())
    }
    pub fn clear(&mut self) {
        self.matcher.clear_active_keys();
    }
}
