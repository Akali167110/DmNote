...
describe('inside 카운터 컴파일러 가드', () => {
  it('InsideCounterLayout은 컴파일되며 산출물에서 시그널 .value를 읽지 않는다', () => {
    const code = compile(
      'packages/editor/src/renderer/components/overlay/counters/InsideCounterLayout.tsx',
    );
  ...
  it("SignalCountDisplay는 'use no memo'로 컴파일 대상에서 제외된다", () => {
    const code = compile(
      'packages/editor/src/renderer/components/overlay/counters/SignalCountDisplay.tsx',
    );
