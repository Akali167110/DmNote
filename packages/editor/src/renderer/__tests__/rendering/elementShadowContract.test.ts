...
describe('기본 요소 눌림 섀도 계약', () => {
  ...
  it('전역 키·노브 규칙이 공개 변수 뒤에 앱 fallback을 둔다', () => {
    const css = readFileSync(
      resolve(process.cwd(), 'packages/editor/src/renderer/styles/global.css'),
      'utf8',
    );
  ...
  it('background 축약 속성을 쓰는 표면도 padding-box 클립을 명시한다', () => {
    const files = [
      'packages/editor/src/renderer/components/shared/KnobFace.tsx',
      'packages/editor/src/renderer/components/shared/graph/GraphPanel.tsx',
    ];
    ...
    for (const file of [
      'packages/editor/src/renderer/components/main/Grid/layers/KnobItem.tsx',
      'packages/editor/src/renderer/components/overlay/counters/OverlayKnobItem.tsx',
    ]) {
      const source = readFileSync(resolve(process.cwd(), file), 'utf8');
