...
const TOKENS = readSource('packages/editor/src/renderer/styles/tokens.css');
...
describe('z-index 층 계약', () => {
  it('그리드 컨테이너가 캔버스 스택을 격리한다', () => {
    const gridSource = readSource(
      'packages/editor/src/renderer/components/main/Grid/core/Grid.tsx',
    );
  ...
  it('크롬 사다리 구간을 맨숫자로 쓰지 않는다', () => {
    const offenders = ['src/renderer', 'packages/editor/src/renderer']
