  ...
  it('패널 외부 피커 3종이 공용 표시 호스트를 거쳐 같은 앵커 정렬을 쓴다', () => {
    ...
    for (const file of files) {
      const source = readFileSync(
        resolve(
          process.cwd(),
          'packages/editor/src/renderer/components/main/Modal/content/pickers',
          file,
        ),
        'utf8',
      );
    ...
    const surface = readFileSync(
      resolve(
        process.cwd(),
        'packages/editor/src/renderer/components/main/Grid/PropertiesPanel/controls/PickerSurface.tsx',
      ),
      'utf8',
    );
