...
패키지 빌드 후 저장소 루트에서 `npm run demo:web-host`를 실행하면 <http://127.0.0.1:4178/>에서 편집 화면을 사용할 수 있습니다. Grid·프로퍼티 패널·툴바·컬러 피커·모달과 별도 iframe의 오버레이를 공통 패키지로 조립합니다. 프리셋 가져오기·내보내기와 브라우저 로컬 저장도 연결합니다. 앱의 설정 화면과 외부 CSS 중계 서버는 이 데모에 포함하지 않습니다.

처음 실행할 때는 `npm run setup:web-wasm`과 `npm run build:packages`로 산출물을 준비합니다. 데모는 빌드된 패키지를 사용하므로 패키지를 수정했다면 다시 빌드한 뒤 데모를 재시작합니다. Ctrl+C로 서버를 종료할 수 있으며, 서버의 임시 파일이 정리되어도 브라우저에 저장한 문서는 유지됩니다. 같은 브라우저에서는 문서당 main 하나만 연결할 수 있습니다. 다른 문서를 열려면 `?document=다른이름`을 사용합니다.

조립 예제는 `scripts/packages/fixtures/web-demo.tsx`와 `web-demo-overlay.tsx`에 있습니다.

미리보기는 하단에서 접고 펼칠 수 있습니다. 상단 경계를 드래그하거나 경계에 포커스한 뒤 위·아래 방향키로 높이를 조절합니다. `확대 보기`는 같은 iframe을 유지한 채 큰 화면으로 표시하고, `축소 보기` 또는 Esc로 돌아옵니다. 디자인 전체가 들어오도록 표시 배율만 조절하며 프리셋 크기는 변경하지 않습니다. 이 배치 구현은 `web-demo-preview.tsx`에 둡니다.

캔버스는 작업 영역의 전체 폭을 사용하고 양쪽 패널이 그 위에 겹쳐 표시됩니다. 왼쪽에는 `CanvasPanel`의 레이어·그리드 탭, 오른쪽에는 `PropertiesPanel includeCanvasPanel={false} visibilityMode="manual"`의 선택 속성을 표시합니다. 양쪽 패널은 처음에 열린 상태이며 각각 수동으로 접을 수 있고 미리보기 옆까지 세로로 이어집니다. 빈 캔버스 클릭이나 선택 변경은 우측 패널을 자동으로 접거나 펼치지 않습니다. 패널을 접거나 펼쳐도 캔버스의 시작 위치와 폭은 유지하며, 하단 미리보기만 열린 패널 사이의 공간을 사용합니다. 미니맵·줌 컨트롤은 왼쪽 패널에 가리지 않도록 이동합니다. `PropertiesPanel`의 기본값은 기존 앱처럼 캔버스 탭과 선택에 따른 자동 표시 동작을 포함합니다.

하단 설정 버튼은 `EditorSettingsModal`을 엽니다. 기존 앱의 카드·행·미리보기·관리 페인을 재사용하며 노트 효과, 키 카운터, 커스텀 CSS, JS 플러그인, 언어만 표시합니다. CSS와 플러그인 파일은 브라우저에서 선택하고 설정은 웹 호스트에 저장합니다.
